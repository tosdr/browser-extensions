...
exports.main = function(options, callbacks) {
	...
	// create storage for services/Points locally
	...
	function getPoint(renderdata){
		Request({
			...
			onComplete: function (dataPoint) {
				...
				ss.storage.points[renderdata[1]] = dataPoint;
				ss.storage.points[renderdata[1]].lastPointUpdate = new Date().getTime();
	...
	panel.port.on("renderDataPoint", function(renderdata) {
		...
		if(!ss.storage.points[renderdata[1]]){
			getPoint(renderdata);
			return;
		}
		
		var daysSinceLastPointUpdate = Math.ceil(Math.abs(new Date().getTime() - ss.storage.points[renderdata[1]].lastPointUpdate) / (1000 * 3600 * 24));
		
		if(daysSinceLastPointUpdate > 13){
			getPoint(renderdata);
			return;	
		}
		
		//Request locally stored point data
		//Load point data from localstorage
		log("Getting point " + renderdata[1] + " from localstorage");
		var renderdataP = [];
        renderdataP[0] = renderdata[0];
        renderdataP[1] = ss.storage.points[renderdata[1]];
		//send data to content script
		panel.port.emit("tosdrpoint", renderdataP);
	});
