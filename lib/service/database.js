...
var Services = Class({
  ...
  isListOutdatedOrFirstTime : function isListOutdatedOrFirstTime() {
    let deferred = defer();
	
    if (typeof ss.storage.lastServicesUpdate === "undefined" || this.daysSince(ss.storage.lastServicesUpdate, 13)) {
      deferred.reject();
    } else { 
		deferred.resolve();
	}
  ...
  set: function set(name, service) {
	  ...
	  if (typeof ss.storage.tosdrServices[name] === "undefined") {
		  ...
		  //Keep lastNotification date
		  if (typeof ss.storage.tosdrServices[name].service.lastNotification != "undefined")
			  lastServiceNotification = ss.storage.tosdrServices[name].service.lastNotification;
		
		  ss.storage.tosdrServices[name] = {};
		  ss.storage.tosdrServices[name].service = service;
		  ss.storage.tosdrServices[name].updated = new Date().getTime();
		  if(lastServiceNotification)
			  ss.storage.tosdrServices[name].service.lastNotification = lastServiceNotification;
	  }
  ...
  notificationOutdated: function notificationOutdated(name) {
    ...
    if (typeof ss.storage.tosdrServices[name].service.lastNotification === "undefined") {
      ...
      if (this.daysSince(ss.storage.tosdrServices[name].service.lastNotification, 13)) { deferred.reject(); }
		  ...
		  //user has been notified recently.
		  deferred.resolve();
	  }      
    }
    return deferred.promise;
