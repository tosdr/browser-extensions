...
exports.main = function(options, callbacks) {
	...
	if(ss.storage.lastServicesUpdate){
		...
		if(daysSinceLastServicesUpdate > 13){
			...
			Request({
				url: "https://tosdr.org/index/services.json",
	...
	function getService(serviceName,serviceIndexData){
		...
		Request({
			url: 'https://tosdr.org/services/' + serviceName + '.json',
	...
	var panel = require("sdk/panel").Panel({
		...
		onShow: function(){
			if(ss.storage.currentActiveService){
				Request({
					url: 'https://tosdr.org/api/1/service/' + ss.storage.currentActiveService + '.json',
					onComplete: function (dataPoint) {
						var serviceData = dataPoint.json;
